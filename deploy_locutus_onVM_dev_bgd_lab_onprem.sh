...
# ./deploy_locutus_onVM_dev_bgd_lab_onprem.sh --manifest ./test_15batch_4conductor_2024oct31.csv.testfun_subbatch01.csv --suffix testfun_subbatch01
# for bash getopts:
# ./deploy_locutus_onVM_dev_bgd_lab_onprem.sh -m ./test_15batch_4conductor_2024oct31.csv.testfun_subbatch01.csv -s testfun_subbatch01
...
# SUDO_CMD=''
# DONE: 3x or so sudo commands already existing in this for onVM deployment; updated to ${SUDO_CMD}
...
if [[ ${DRY_RUN} -eq 0 ]]; then
    ...
    echo "DRY_RUN=${DRY_RUN}: **NOT** actually calling the start script: ${SUDO_CMD} -E ./scripts/run_docker_onVM_dev_bgd_lab_onprem.sh"
...
if [[ ${DRY_RUN} -eq 0 ]]; then
    ...
    ${SUDO_CMD} -E ./scripts/run_docker_onVM_dev_bgd_lab_onprem.sh
    ...
    echo "DRY_RUN: **NOT** actually calling the start script: ${SUDO_CMD} -E ./scripts/run_docker_onVM_dev_bgd_lab_onprem.sh"
